...
// 释放frame_rate_data选项数据数组所有元素并将数组大小设置为0
...
// 设置frame_rate_data范围数据数组所有元素并将数组大小设置为0
...
/* ------------------------------------------------------------------------- */
// 对props->last指向的内存赋值后再把props->last指向p->next
...
// 检索props的最上级父节点所在的链表中是否相同name的元素
...
// 检索parent最高节点与group节点是否存在相同name
...
// 在父节点props末尾处创建一个子节点(name，desc，type)，再把这个子节点当作group组节点的父节点
...
// 返回类型为Group的元素包含的content链表，否则为null
