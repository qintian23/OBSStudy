	       ...
	       obs_data_t *settings, size_t mixer_idx, obs_data_t *hotkey_data)
{
	...
	struct obs_encoder_info *ei = find_encoder(id);// 根据ID找到编码器信息
	bool success;
...
{
	if (!obs_encoder_valid(encoder, "obs_encoder_update"))// 检测encoder是否为NULL
		return;

	obs_data_apply(encoder->context.settings, settings);// 将settings所有的item数据复制到encoder中
	...
	if (encoder->info.update)
		encoder->reconfigure_requested = true;// 在其它线程中更新
}
