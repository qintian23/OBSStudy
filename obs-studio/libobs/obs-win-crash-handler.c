...
static inline bool get_dbghelp_imports(struct exception_handler_data *data)
{
	...
	GET_DBGHELP_IMPORT(sym_initialize, "SymInitialize");// 初始化一个进程的符号表
...
void initialize_crash_handler(void)
{
	...
	if (!initialized) {
		//一个未被调试的进程发生异常，并且该异常进入未处理异常过滤器，该过滤器将调用lpTopLevelExceptionFilter参数指定的异常过滤函数。
