...
obs_source_t *obs_weak_source_get_source(obs_weak_source_t *weak)
{
	...
	if (obs_weak_ref_get_ref(&weak->ref))// 引用计数加1
		return weak->source;
...
/// <param name="mixers">输出混合数量</param>
					     ...
					     size_t sample_rate, size_t size)
{
	...
	if (source->audio_input_buf[0].size < size) {// 不够输出一帧
...
{
	if (!source->audio_output_buf[0][0]) {// 检测源是否分配了音频输出缓冲区
		source->audio_pending = true;
		return;
	}

	if (source->info.audio_render) {// 是否为自定义源
		if (!source->context.data) {
			source->audio_pending = true;
			return;
		}
