...
struct profile_root_entry {
	...
	const char *name;
	profile_entry *entry;   // 起始函数所有运行的时间信息
	profile_call *prev_call;// 代表之前调用过这个起始函数
};
...
/// <summary>
/// 获取一个程序运行的起始函数，如果没有则添加一个起始函数
/// </summary>
/// <param name="name">起始函数名称</param>
/// <returns></returns>
...
static void merge_context(profile_call *context)
{
	...
	free_call_context(prev_call);// 融合完成释放掉之前的调用
}
...
void profile_start(const char *name)
{
	...
	// 如果是起始函数则新建，否则便添加到子函数集合中（子函数代表调用的函数）
	if (new_call.parent) {
...
void profile_end(const char *name)
{
	...
	if (call->name != name) {// 名称不相等，可能调用了新的函数
		blog(LOG_ERROR,
		...
		while (parent && parent->parent && parent->name != name)// 向上查询，查找是否为上级函数
			parent = parent->parent;

		if (!parent || parent->name != name)
			return;

		while (call->name != name) {// 如果找到是上级函数，结束上级函数调用其它函数的运行时间统计
			profile_end(call->name);
			call = call->parent;
		}
	}

	thread_context = call->parent;// 当前函数结束，运行时间点更新为上级函数

	call->end_time = end;// 结束时间统计
#ifdef TRACK_OVERHEAD
	call->overhead_end = os_gettime_ns();
#endif

	if (call->parent)// 如果不是起始函数（一般线程的开始）则直接返回
		return;
