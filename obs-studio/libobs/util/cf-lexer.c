...
/// <summary>
/// 跳过多个splice，splice为首字符是‘\\’，第二个字符为换行符
/// </summary>
...
/// <summary>
/// 处理注释字符，输出到out_token中
/// </summary>
/// <param name="lex"></param>
/// <param name="out_token"></param>
/// <returns></returns>
...
/// <summary>
/// 获取token类型为字符串，遇到delimiter符号或者换行符停止
/// </summary>
/// <param name="lex"></param>
/// <param name="out_token"></param>
/// <param name="delimiter"></param>
/// <param name="allow_escaped_delimiters"></param>
				    ...
				    struct cf_token *out_token)
{
	...
	if (ch == '<' && cf_lexer_is_include(lex)) {// 是否为系统include <**>
...
/// <summary>
/// 对include define undef ifdef ifndef else endif等字符串进行处理
/// </summary>
/// <param name="pp"></param>
/// <param name="if_block"></param>
/// <param name="p_cur_token"></param>
/// <returns></returns>
static bool cf_preprocessor(struct cf_preprocessor *pp, bool if_block,
			    struct cf_token **p_cur_token)
{
				 ...
				 struct cf_token **p_cur_token)
{
	...
	while (cur_token->type != CFTOKEN_NONE) {// 不是最后一个
