...
/// <summary>
/// 从lexer中offset偏移处开始，获取以end作为结束字符的一段字符串，未用内存复制
/// </summary>
			    ...
			    struct strref *value)
{
	...
	item.name = bstrdup_n(name->array, name->len);// 字符拷贝
				 ...
				 struct lexer *lex)
{
	...
	while (lexer_getbasetoken(lex, &token, PARSE_WHITESPACE)) {
			...
			if (*token.text.array == '#') {
				do {
					if (!lexer_getbasetoken(
						    ...
						    PARSE_WHITESPACE))
						return;
				} while (!is_newline(*token.text.array));// 移至下一个换行符
		...
		strref_copy(&name, &token.text);// 引用字符串指针，标志name首字符
		if (!config_parse_string(lex, &name, '='))// 从name首字符开始，获取以=字符结束的字符段
			continue;

		strref_clear(&value);
		config_parse_string(lex, &value, 0);// 从=字符开始，以换行符为结束的字符段
...
static void parse_config_data(struct darray *sections, struct lexer *lex)
{
	...
	while (lexer_getbasetoken(lex, &token, PARSE_WHITESPACE)) {
		...
		while (token.type == BASETOKEN_WHITESPACE) { // 跳过换行符和空格符
			if (!lexer_getbasetoken(lex, &token, PARSE_WHITESPACE))
				return;
		}
