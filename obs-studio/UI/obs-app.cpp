...
/// <summary>
/// 确定用户文件夹是否创建，如没有则创建; eg: obs-studio/basic
/// </summary>
/// <returns></returns>
...
/// <summary>
/// 确定用户基础配置文件夹是否创建，如没有则创建; eg: obs-studio/basic/profiles
/// </summary>
/// <returns></returns>
...
bool OBSApp::InitLocale()
{
	...
	if (!locale.empty())// 设置QT本地化输出
		QLocale::setDefault(QLocale(
			QString::fromStdString(locale).replace('-', '_')));

	string englishPath;
	if (!GetDataFilePath("locale/" DEFAULT_LANG ".ini", englishPath)) {// 获取英文配置文件路径
		OBSErrorBox(NULL, "Failed to find locale/" DEFAULT_LANG ".ini");
		return false;
	}

	textLookup = text_lookup_create(englishPath.c_str());// 解析并读取英文配置文件
	if (!textLookup) {
	...
	if (!userLocale && defaultLang) {
		for (auto &locale_ : GetPreferredLocales()) {// 配置电脑首选语言
			if (locale_ == lang)
				return true;
...
bool OBSApp::InitTheme()
{
	...
	if (!themeName)
		...
		themeName = config_get_string(globalConfig, "General",// 默认配置为Dark
					      "CurrentTheme");
	if (!themeName)
...
void OBSApp::AppInit()
{
	ProfileScope("OBSApp::AppInit");// 对函数AppInit进行时间统计
...
#ifdef _WIN32
	bool disableAudioDucking =
		config_get_bool(globalConfig, "Audio", "DisableAudioDucking");// 默认配置
	if (disableAudioDucking)
		DisableAudioDucking(true);// 开启
#endif
	...
	UpdateHotkeyFocusSetting(false);// 关闭热键
...
/// UI线程执行任务
/// </summary>
/// <param name="task">函数</param>
/// <param name="param">函数参数</param>
/// <param name="wait">是否等待执行完任务</param>
...
bool OBSApp::OBSInit()
{
	...
	setAttribute(Qt::AA_UseHighDpiPixmaps);// 界面使用高DPI
#endif

	qRegisterMetaType<VoidFunc>();//对自定义类型进行注册
	...
	// 最后一个窗口关闭时，主函数main不退出
	setQuitOnLastWindowClosed(false);

	mainWindow = new OBSBasic();

	mainWindow->setAttribute(Qt::WA_DeleteOnClose, true);// 设置关闭即窗口销毁
	connect(mainWindow, SIGNAL(destroyed()), this, SLOT(quit()));// 连接主窗口关闭调用程序的退出事件
...
/// <summary>
/// 如果文件数过多，按照创建时间先后来删除文件
/// </summary>
/// <param name="has_prefix"></param>
...
/// </summary>
/// <param name="has_prefix"></param>
/// <param name="subdir_to_use"></param>
...
static void create_log_file(fstream &logFile)
{
	...
	BPtr<char> path(GetConfigPathPtr(dst.str().c_str()));// 获取绝对路径

#ifdef _WIN32
	BPtr<wchar_t> wpath;
	os_utf8_to_wcs_ptr(path, 0, &wpath);
	logFile.open(wpath, ios_base::in | ios_base::out | ios_base::trunc);// 创建日志文件
#else
...
//#define PROFILEER_FUN(x)  ScopeProfiler prof{x};
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	auto profilerNameStore = CreateNameStore();// 动态开辟一个vector用于存储计时函数名称

	std::unique_ptr<void, decltype(ProfilerFree)> prof_release(// 保存函数运行时间，释放资源
		static_cast<void *>(&ProfilerFree), ProfilerFree);

	profiler_start();
	profile_register_root(run_program_init, 0);
	// run_program_init函数分析器用于获取有关程序性能和效率的信息。
	...
	try {
		QAccessible::installFactory(accessibleFactory);// 音量控件修改

		bool created_log = false;
		// OBS框架初始化，主要是对配置文件的操作
		program.AppInit();
		delete_oldest_file(false, "obs-studio/profiler_data");// 删除过多的程序分析文件

		OBSTranslator translator;
		program.installTranslator(&translator);// 系统会产生语言更改的信号
...
/// <summary>
/// 修改当前进程可以调式另一个账户的进程内存
/// </summary>
/// <param name=""></param>
static void load_debug_privilege(void)
{
	// 需要启用或禁用访问令牌中的权限 | 需要查询访问令牌。
	const DWORD flags = TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY;
	TOKEN_PRIVILEGES tp;
	HANDLE token;
	LUID val;
	// token 指向句柄的指针，该句柄在函数返回时标识新打开的访问令牌
...
int main(int argc, char *argv[])
{
...
#ifdef _WIN32
	obs_init_win32_crash_handler();// 用于window中断处理函数调用时，对系统信息的日志打印
	SetErrorMode(SEM_FAILCRITICALERRORS);// 屏蔽系统错误窗口
	load_debug_privilege();
	base_set_crash_handler(main_crash_handler, nullptr);// 设置自定义中断处理函数

	const HMODULE hRtwq = LoadLibrary(L"RTWorkQ.dll");
	if (hRtwq) {
		typedef HRESULT(STDAPICALLTYPE * PFN_RtwqStartup)();
		PFN_RtwqStartup func =
			(PFN_RtwqStartup)GetProcAddress(hRtwq, "RtwqStartup");// 用于开启桌面音频采集实时工作队列
		func();
	}
#endif

	base_get_log_handler(&def_log_handler, nullptr);// 获取日志打印回调

#if defined(USE_XDG) && defined(IS_UNIX)
	move_to_xdg();
#endif

	obs_set_cmdline_args(argc, argv);// 缓存参数内容
	...
	curl_global_init(CURL_GLOBAL_ALL);// 通信协议FTP、FTPS、HTTP、HTTPS等
	...
	if (hRtwq) {
		...
		PFN_RtwqShutdown func =
			(PFN_RtwqShutdown)GetProcAddress(hRtwq, "RtwqShutdown");// 用于关闭桌面音频采集实时工作队列
		func();
		FreeLibrary(hRtwq);
	}
#endif
