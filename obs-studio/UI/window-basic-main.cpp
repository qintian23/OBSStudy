...
/// <summary>
/// 停靠窗口下拉按钮与指定dock的绑定，包含按钮的触发事件和指定dock显示或隐藏触发事件
/// </summary>
	...
	: OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	statsDock->setFeatures(QDockWidget::DockWidgetClosable | // 可关闭
			       QDockWidget::DockWidgetMovable |  // 可移动
			       QDockWidget::DockWidgetFloatable);// 可漂浮
	statsDock->setWindowTitle(QTStr("Basic.Stats"));
	addDockWidget(Qt::BottomDockWidgetArea, statsDock);// 下边停靠
	statsDock->setVisible(false);
	statsDock->setFloating(true);// 漂浮状态
	...
	ui->scenes->setAttribute(Qt::WA_MacShowFocusRect, false);// 关闭控件边框显示
	ui->sources->setAttribute(Qt::WA_MacShowFocusRect, false);

	bool sceneGrid = config_get_bool(App()->GlobalConfig(), "BasicWindow",
					 "gridMode");
	ui->scenes->SetGridMode(sceneGrid);// 设置网格显示

	ui->scenes->setItemDelegate(new SceneRenameDelegate(ui->scenes));// 设置修改Item委托  由EditSceneName触发
	...
	installEventFilter(shortcutFilter);// 安装快捷键过滤器
	...
	connect(cpuUsageTimer.data(), SIGNAL(timeout()), ui->statusbar,
		SLOT(UpdateCPUUsage()));
	cpuUsageTimer->start(3000);// 定时每3秒更新进程CPU使用率和帧率

	diskFullTimer = new QTimer(this);
	connect(diskFullTimer, SIGNAL(timeout()), this,
		SLOT(CheckDiskSpaceRemaining()));// 连接检查磁盘剩余容量事件

	renameScene = new QAction(ui->scenesDock);
	renameScene->setShortcutContext(Qt::WidgetWithChildrenShortcut);// 设置父窗口或者子窗口为focus状态时才生效
	connect(renameScene, SIGNAL(triggered()), this, SLOT(EditSceneName()));
	ui->scenesDock->addAction(renameScene);// 添加场景窗口右键重命名下拉按钮
	...
	ui->sourcesDock->addAction(renameSource);// 添加来源窗口右键重命名下拉按钮
...
#else
	renameScene->setShortcut({Qt::Key_F2});// 设置场景重命名快捷键
	renameSource->setShortcut({Qt::Key_F2});// 设置来源重命名快捷键
#endif
	...
	auto addNudge = [this](const QKeySequence &seq, const char *s) {
		...
		nudge->setShortcutContext(Qt::WidgetShortcut);// 设置父窗口为focus状态时才生效
	...
	ui->actionMainUndo->setShortcutContext(Qt::ApplicationShortcut);// 设置应用程序为focus状态时才生效
	...
	if (geometry != NULL) {
		...
		QRect windowGeometry = normalGeometry();// 获取窗口位置，大小
	...
	ui->previewLabel->style()->polish(ui->previewLabel); // 初始化preview上方控件的外观
	// 工作模式
...
void OBSBasic::copyActionsDynamicProperties()
{
	...
	for (QAction *x : ui->scenesToolbar->actions()) {
		...
		for (QByteArray &y : x->dynamicPropertyNames()) {
			temp->setProperty(y, x->property(y)); // addIconSmall  removeIconSmall upArrowIconSmall downArrowIconSmall
		}
	}
	...
	for (QAction *x : ui->sourcesToolbar->actions()) {
		...
		for (QByteArray &y : x->dynamicPropertyNames()) {
			temp->setProperty(y, x->property(y));// addIconSmall  removeIconSmall propertiesIconSmall upArrowIconSmall downArrowIconSmall
		}
	}
}
...
static const double scaled_vals[] = {1.0,         1.25, (1.0 / 0.75), 1.5,
				     ...
				     2.5,         2.75, 3.0,          0.0};
/// <summary>
/// 检查是否存在CookidId，没有则随机生成一个
/// </summary>
extern void CheckExistingCookieId();

bool OBSBasic::InitBasicConfigDefaults()
{
	QList<QScreen *> screens = QGuiApplication::screens();// 获取屏幕显示器
	...
	QScreen *primaryScreen = QGuiApplication::primaryScreen();// 获取主屏幕
	...
	config_set_default_string(basicConfig, "SimpleOutput", "Preset", 
				  "veryfast");
	config_set_default_string(basicConfig, "SimpleOutput", "NVENCPreset",// 硬件编码质量
				  "hq");
	...
	/* use a default scaled resolution that has a pixel count no higher 1280 * 720
	 * than 1280x720 */
...
/// <summary>
/// 对场景Item进行重命名，如果名称存在相同或者为空，则不进行重命名。加入撤销事件队列和重做事件队列
/// </summary>
/// <param name="parent"></param>
/// <param name="listWidget"></param>
			       ...
			       QAbstractItemDelegate::EndEditHint endHint)
{
	...
	ui->scenesDock->addAction(renameScene);// 重命名完成后重新加入鼠标右键重命名选项
...
void OBSBasic::StartRecording()
{
	...
	if (!OutputPathValid()) {// 路径不存在
		OutputPathInvalidMessage();
		ui->recordButton->setChecked(false);
		return;
	}

	if (LowDiskSpace()) {// 检测磁盘空间是否足够
		DiskSpaceMessage();// 返回磁盘空间不够消息
		ui->recordButton->setChecked(false);
		return;
	}

	if (api)
		api->on_event(OBS_FRONTEND_EVENT_RECORDING_STARTING);// 目前没看到处理什么

	SaveProject();// 保存配置 obs-studio/basic/scenes/SceneCollectionFile.json
...
void OBSBasic::on_recordButton_clicked()
{
	if (outputHandler->RecordingActive()) {
		...
		if (confirm && isVisible()) {
			...
			if (button == QMessageBox::No) {// 取消
				ui->recordButton->setChecked(true);// 设置按钮为选中状态
				return;
			}
		}
		StopRecording();
