...
SimpleOutput::SimpleOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	if (strcmp(encoder, SIMPLE_ENCODER_QSV) == 0) {//创建intel集显编码器
		LoadStreamingPreset_h264("obs_qsv11");

	} else if (strcmp(encoder, SIMPLE_ENCODER_AMD) == 0) {//创建AMD独显编码器
		LoadStreamingPreset_h264("amd_amf_h264");

	} else if (strcmp(encoder, SIMPLE_ENCODER_NVENC) == 0) {//创建NVENC独显编码器
		const char *id = EncoderAvailable("jim_nvenc") ? "jim_nvenc"
							       : "ffmpeg_nvenc";
		LoadStreamingPreset_h264(id);

	} else {
		LoadStreamingPreset_h264("obs_x264");// 创建x264软编码
	}
...
{
	OBSDataAutoRelease settings = obs_data_create();
	obs_data_set_int(settings, "crf", crf);// 写入crf值
