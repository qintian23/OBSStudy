# Set OS-specific constants in non-deprecated way 设置编译平台选择
...
# set_property 链接：https://cmake.org/cmake/help/latest/command/set_property.html
...
# Helper function to set up runtime or library targets 设置程序或者运行库安装路径
function(setup_binary_target target)
  # Set up installation paths for program install
  # install See:https://cmake.org/cmake/help/latest/command/install.html?highlight=install
  # 定义安装规则 TARGETS目标文件 RUNTIME可执行文件 LIBRARY动态库 ARCHIVE静态库 PUBLIC_HEADER头文件
  # DESTINATION 磁盘上要安装文件的目录 COMPONENT安装组件名称 NAMELINK_COMPONENT目标链接文件（快捷方式）
  install(
    TARGETS ${target}
  ...
  # EXCLUDE_FROM_ALL 属性不会自动编译，只能手动编译
  install(
    TARGETS ${target}
  ...
  # 添加指令 
  # POST_BUILD 生成事件后执行 COMMENT 在构建时执行命令之前显示给定的消息。
  add_custom_command(
    TARGET ${target}
    POST_BUILD
    COMMAND
